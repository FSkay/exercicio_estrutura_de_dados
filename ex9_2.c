...
int main()
{
      ...
      while(seguir == 's')
      {
      printf("    *** Cadastro de funcionarios (lucassweb) *** \n\n");
      printf("        1) NOVO \n");
      printf("        2) REMOVER \n");
      printf("        3) LISTAR \n");
      printf("        4) PESQUISAR \n");
        ...
        case 3:
             {
               if(topo == 0)
               {
                 printf("nao ha dados no sistema!\n\n");
               }
               else
               {
...
{    
     ...
     setbuf(stdin, NULL);
     printf(" SALARIO: ");
     scanf("%f", &funcionario[topo].salario); setbuf(stdin, NULL);
          
     printf(" CARGO: ");
     fgets(funcionario[topo].cargo,30,stdin);
     funcionario[topo].cargo[strlen(funcionario[topo].cargo)-1]; setbuf(stdin, NULL);
    
     printf(" IDADE: ");
     scanf("%d", &funcionario[topo].idade); setbuf(stdin, NULL);
     
     printf(" SEXO: ");
     fgets(funcionario[topo].sexo,2,stdin);
     funcionario[topo].sexo[strlen(funcionario[topo].sexo)-1]; setbuf(stdin, NULL);
     printf("\n");
